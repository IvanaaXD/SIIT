...
class AtletskaStaza {
    ...
    public:
		...
		povratna_vrednost skaci() {
			...
			this_thread::sleep_for(seconds(SKOK+1));
		...
		povratna_vrednost baciKoplje() {
			...
			this_thread::sleep_for(seconds(BACANJE+1));
			auto zavrsio = steady_clock::now();
...
void atletika(AtletskaStaza& a, int disc) {
    ...
    unique_lock<mutex> l(terminal);
    if (disc == 0) {
        cout << "Skakac " << this_thread::get_id() << " je skocio " << p.duzina << " metara i trebalo mu je " << p.trajanje.count() << " sekundi"  <<  endl;
    } else if (disc == 1) {
        cout << "Bacac " << this_thread::get_id() << " je bacio " << p.duzina << " metara i trebalo mu je " << p.trajanje.count() << " sekundi"  <<  endl;
    }
}

int main() {

    AtletskaStaza a(SKAKACI, BACACI);
    int n = BACACI + SKAKACI;
    thread t[BACACI + SKAKACI];

    for (int i = 0; i < n; i++) {
        t[i] = thread(atletika, ref(a), (AtletskaStaza::discpilina)(i%2));
    }
