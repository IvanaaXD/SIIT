...
def plot_correlation_for_col(df, col_name):
    ...
    sorted_col_corr = correlation_matrix[col_name].sort_values(
        ascending=True)  # indeksiramo kolonu i soritramo vrednosti
...
def visualize_principal_components(principal_components: np.ndarray, n_principal_components: int, target_col=None):
    ...
    if n_principal_components == 2:
        ...
        fig.update_layout(title='Vizualizacija glavnih komponenti',
                          scene=dict(xaxis_title="PC1", yaxis_title="PC2", zaxis_title="PC3",
                                     xaxis_autorange='reversed', yaxis_autorange='reversed'))
        fig.show()

    else:
        raise Exception('number of principal components must 2  or 3')
