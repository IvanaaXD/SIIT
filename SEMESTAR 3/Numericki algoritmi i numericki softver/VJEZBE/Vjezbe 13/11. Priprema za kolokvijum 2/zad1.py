...
if __name__ == '__main__':
    ...
    nan_months = df[df.isna().values]  # trazimo mesece sa nedostajucim vrednostima
    print(nan_months)  # ispisi mesece koje nedostaju da proveris da se podudaraju sa grafikom.

    # izdvajamo podatke od pocetka do prve nedostajuce vrednosti
    nan_months = df[df.isna().values]
    first_nan_month = nan_months.index[0]  # 1960-05-01
    nan_months_train_df = df.loc[:first_nan_month].iloc[:-1]  # uzmi sve do prve NaN vrednost (n)

    # pravimo model da popuni nedostajuce vrednosti
    ...
    # odredjujemo parametre modela
    # sa PACF grafika vidimo da je zacajna vrednost 12, pa koristimo AR model sa p=12
    # takodje, za stacionarnost je potrebno samo 1x diferenciranje, pa je d=1
