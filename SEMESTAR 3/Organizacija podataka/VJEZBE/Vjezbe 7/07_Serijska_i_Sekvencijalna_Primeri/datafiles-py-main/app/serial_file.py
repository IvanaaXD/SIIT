...
class SerialFile(BinaryFile):
    ...
    def init_file(self):
        with open(self.filename, "wb") as f:
            block = self.blocking_factor*[self.get_empty_rec()]  # inicijalizacija datoteke podrazumeva unos bloka
            # koji sadrzi prazne slogove
            self.write_block(f, block)

    def insert_record(self, rec):
        if self.find_by_id(rec.get("id")):  # provera da li vec postoji slog sa zadatim id-jem - svakom unosu
            # prethodi pretraga!
    ...
    def delete_by_id(self, id):
        ...
        with open(self.filename, "rb+") as f:
            while True:
                ...
                i = rec_idx
                while i < self.blocking_factor-1:  # brisemo block[rec_idx] tako sto sve nakon njega pomeramo za
                    # jedno mesto ka levo
                    block[i] = block[i+1]
                    i += 1
                ...
                next_block = self.read_block(f)  # ako nije poslednji blok, onda pomeranje mora da se nastavi i u
                # narednim blokovima, dok ne stignemo do poslednjeg
        ...
        if next_block and next_block[0].get("id") == self.empty_key:  # ako smo vrsili pomeranje iz poslednjeg bloka,
            # i ako je sada kod njega na prvom mestu prazan slog - mozemo osloboditi memoriju
