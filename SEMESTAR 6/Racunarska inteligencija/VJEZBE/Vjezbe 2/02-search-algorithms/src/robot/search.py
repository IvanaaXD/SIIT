...
class IterativeDepthFirstSearch(Search):
    def select_state(self, states):
        # TODO 2 - Implementirati pretragu itaritvni prvi u dubinu (IDFS)
        pass

class UniformCostSearch(Search):
    def select_state(self, states):
        # TODO 3 - Impelentirati uniform cost search (UCS)

        min_state = None
        min_value = math.inf

        for state in states:
            if state.get_current_cost() < min_value:
                min_value = state.get_current_cost()
                min_state = state
...
class GreedySearch(Search):
    def select_state(self, states):
        ...
        for state in states:
            if state.get_cost_estimate() < min_value:
                min_value = state.get_cost_estimate()
                min_state = state
...
class AStarSearch(Search):
    def select_state(self, states):
        ...
        for state in states:
            cost = state.get_cost_estimate() + state.get_current_cost()
            if cost < min_value:
                min_value = cost
                min_state = state
