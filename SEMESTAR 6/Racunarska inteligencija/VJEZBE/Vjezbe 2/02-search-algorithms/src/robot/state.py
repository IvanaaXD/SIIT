...
class RobotState(State):
    ...
    def __init__(self, board: Board, parent: State=None, position: tuple=None, goal_position: tuple=None):
        ...
        self.checkpoints = tuple(cp for cp in self.checkpoints if cp != self.position)

        if self.position in self.teleports:
            self.teleport = True
            self.teleports = tuple(t for t in self.teleports if t != self.position)

        if self.parent.teleport and self.parent is not None:
            self.teleport = False
    ...
    def get_legal_positions(self):
        ...
        if self.teleport:
            new_positions.append(self.teleports[0])
