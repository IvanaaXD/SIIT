...
def get_sequence_length(number, sequence):
    """
    Pomoćna funkcija u primeru collatz_sequence_list_caching_all
    :param number: broj čija dužina razvoja se izračunava
    :param sequence: lista u kojoj su zabeležene do sada izračunate dužine razvoja
    :return: dužina razvoja
    """
    if number == 1:
        return 1

    if number < len(sequence) and sequence[number] != 0:
        return sequence[number]

    next_number = generate_next(number)

    value = 1 + get_sequence_length(next_number, sequence)
